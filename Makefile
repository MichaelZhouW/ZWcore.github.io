...
gh-pages:
	cd 127.0.0.1:8050 && touch .nojekyll && git init && git add * && git add .nojekyll && git commit -m "update" && git remote add origin  https://github.pie.apple.com/wzhou24/wzhou24.github.io.git && git push -f origin master
